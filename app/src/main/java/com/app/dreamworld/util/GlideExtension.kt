package com.app.dreamworld.util
...
fun ImageView.loadGIF(context: Context, url: String?, placeHolder: Int? = R.drawable.logo) {
    if (placeHolder != null) {
 ...
 */
fun ImageView.loadImage(context: Context, url: String?, placeHolder: Int? = R.drawable.logo, catchImage: Boolean = false) {
    if (placeHolder != null) {
        Glide.with(context).load(url).centerCrop().placeholder(placeHolder).diskCacheStrategy(
            if (catchImage) DiskCacheStrategy.ALL else DiskCacheStrategy.NONE).timeout(120000).into(this)
    } else {
...
fun ImageView.loadBlurImage(url: String = "", placeHolder: Int? = R.drawable.logo) {
...
fun ImageView.loadImage(context: Context, url: String?, catchImage: Boolean = false) {
    Glide.with(context).load(url).placeholder(R.drawable.logo).diskCacheStrategy(
        if (catchImage) DiskCacheStrategy.ALL else DiskCacheStrategy.NONE).timeout(120000).into(this)
}
 ...
 * Extention function for load image with glide
 */
fun ImageView.loadImage(context: Context, url: Uri?, placeHolder: Int? = R.drawable.logo, catchImage: Boolean = false) {
    if (placeHolder != null) {
        Glide.with(context).load(url).centerCrop().placeholder(placeHolder).diskCacheStrategy(
            if (catchImage) DiskCacheStrategy.ALL else DiskCacheStrategy.NONE).timeout(120000).into(this)
    } else {
 ...
 * Extention function for load image with glide
 */
fun ImageView.loadRoundedCornerCircleImage(context: Context, url: String?, placeHolder: Int = R.drawable.logo,
                                           radius: Int = context.resources.getDimension(com.intuit.sdp.R.dimen._9sdp).toInt(),
...
fun ImageView.loadRoundedCornerCircleImage(context: Context, uri: Uri?, placeHolder: Int = R.drawable.logo,
                                           radius: Int = context.resources.getDimension(com.intuit.sdp.R.dimen._9sdp).toInt(),
                                           catchImage: Boolean = false) {
...
fun ImageView.loadCircleCropImage(context: Context, image: String, placeHolder: Int? = R.drawable.logo,
                                  catchImage: Boolean = false) {
    if (placeHolder != null) {
 ...
 * Extention function for load image with glide
 */
fun ImageView.loadCircleImage(context: Context, url: String?, placeHolder: Int = R.drawable.logo,
                              catchImage: Boolean = false) {
    Glide.with(context).load(url).apply(RequestOptions.circleCropTransform().placeholder(placeHolder).dontAnimate())
        .diskCacheStrategy(if (catchImage) DiskCacheStrategy.ALL else DiskCacheStrategy.NONE).timeout(120000).into(this)
